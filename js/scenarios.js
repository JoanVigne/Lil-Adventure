...
function edmundScenario() {
    choice(`<h4>Edmun: ?</h4><p>So if i mix this ingredient with this one... it is killing this plant even faster!<br>
    ...
    This city mayor is retarded as f*ck to sentence everybody to death.<br></p>`,
        `Yes i am.`, truth,
        `No no, i am here to help`, lie);
}
function truth() {
    closePromptBox();
    choice(`<h4>Edmund: </h4><p> I see, honesty, good quality.<br>
    Well, try to be honest with my pet now. <br>
    Gorilla ! ATTACK !</p>`,
        `Throw a peace of meat to him`, poop,
        `Prepare to fight`, fightGorilla);
}
function poop() {
    closePromptBox();
...
function lie() {
    closePromptBox();
    choice(`<h4>Emdund: </h4><p> Oh, a new member of my big project!<br>
    ...
    Then, the second phase of my master-plan can begin!<br></p>
     Anyway, come closer.`,
        `Obeying`, obeyingEdmund1,
        `I refuse to obey you, Evil!`, truth);
}

function obeyingEdmund1() {
    closePromptBox();
    choice(`<h4>Edmund:</h4><p> Your blade is full of blood... I can see that you passed through a lot. <br>
    Give it to me, so i can collect the blood of those dead, it can be usefull for my next potions.<br></p>`,
        `Obeying`, obeyingEdmund2,
        `Surprise attack on Edmund`, surpriseAttack);
}
...
function obeyingEdmund2() {
    closePromptBox();
    choice(`<p> By giving him your weapon nicely, you can notice that the gorilla is less vigilent... <br>
